...
class Case160Service {
  ...
  async getSearchCase( casesearch, type){
    ...
    const url3 = `http://${ip_webapp}/cojCase/api/v1/caseLit/listByCaseId/`;
    ...
    try {
      ...
      const res3 = await axios.get(url3+response.data.data[0].caseId, config);

      return  { 
        "caseId": response.data.data[0], 
        "caseBlack": res1.data.data,
        "caseRed": res2.data.data,
        "caseLit": res3.data.data
      } ;
  ...
  async getJudList (){
    ...
    const url = `http://${ip_webapp}/cojLookup/api/v1/judges/search?version=1&offset=0&limit=-1&sort=superId&dir=asc`;
    let postBody = {"version":1,"flagJudge":1,"judgeStatus":1,"activeFlag":1};
    const config = { headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' } };
    try {
      const response = await axios.post(url, postBody, config);
  ...
  async getSubdistricts (id_address){
    let token = localStorage.getItem("TOKEN_160");
    if(!token){
      await this.setToken();
      token = localStorage.getItem("TOKEN_160");
    }
    const url = `http://${ip_webapp}/cojLookup/api/v1/subdistricts/${id_address}?version=1&limit=-1`;
    const config = { headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' } };
    try {
